...
class TopicsOverTime:
    ...
    def GetPnasCorpusAndDictionary(self, corpus, timeFile, stopwords_path):
        documents = []
        timestamps = []
        dictionary = set()
        stopwords = set()
        ...
        count_doc = 0
        # docs = corpus.split('.\n')
        # for doc in fileinput.input(corpus):
        #     doc = doc[:len(doc)-2]
        ...
        for t in fileinput.input(timeFile):
        ...
        for iteration in range(par['max_iterations']):
            for d in range(par['D']):
                for i in range(par['N'][d]):
                    ...
                    # print(list(np.random.multinomial(1, topic_probabilities, size=1)[0]))
