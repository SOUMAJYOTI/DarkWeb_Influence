...
def main():
    ...
    w2v_feat = pickle.load(open('../../../darkweb_data/4_23/word2vec_train_model_d50_min3.pickle', 'rb'))
    ...
    for idx_fold in range(1, 5): #range(len(train_fold)):
        print('Fold: ', idx_fold, '\n')
        ...
        for col_p1 in range(3, 12):
            ...
            for idx_trans in range(11):
                """ PHASE 1 FEATURES"""
                docs_unlabel = pickle.load(open('../../../darkweb_data/4_23/unlabeled_final/docs_'
                ...
                Y_train_final_3D = np.array([-1] * X_train_pos_3D.shape[0] + [1] * X_train_neg_3D.shape[0])
                ...
                Y_train_inp = np.array([-1]*X_train_pos.shape[0] + [1]*X_train_neg.shape[0])
                ...
                for idx_prob in range(pred_proba.shape[0]):
                    ...
                    if lab_inst == 0:
                        if pred_proba[idx_prob, 0] > .90:
                            X_safeU.append(X_unlabel_3D[idx_prob])
                            Y_safeu.append(-1)
                    else:
                        if pred_proba[idx_prob, 1] > .90:
                            X_safeU.append(X_unlabel_3D[idx_prob])
                            Y_safeu.append(1)
                ...
                # print(Y_train[Y_train == 1].shape[0] / Y_train.shape[0])
                # print(Y_safeu[Y_safeu == 1].shape[0]/ Y_safeu.shape[0])
                ...
                # print(X_train_final.shape, X_safeU.shape)
            ...
            print(Y_train_final[Y_train_final == 1].shape[0] / Y_train_final.shape[0])
            # print(Y_test[Y_test == 1].shape[0] / Y_test.shape[0])
            # print("Sample shapes: X_train, X_test: ", X_train_3D.shape, X_test_3D.shape,  "\n")
