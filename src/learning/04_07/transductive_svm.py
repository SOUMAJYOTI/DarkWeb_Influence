...
from qns3vm import QN_S3VM_Dense
...
class SKTSVM(BaseEstimator):
    ...
    def __init__(self, kernel='RBF', C=1e-4, gamma=0.5, lamU=0.5, probability=False):
    ...
    def fit(self, labeledX, labeledy, unlabeledX):  # -1 for unlabeled
        ...
        labeledX = labeledX.tolist()
        labeledy = labeledy.tolist()
        unlabeledX = unlabeledX.tolist()

        if 'rbf' in self.kernel.lower():
            self.model = QN_S3VM_Dense(labeledX, labeledy, unlabeledX, self.random_generator, lam=self.C, lamU=self.lamU,
                                 kernel_type="RBF", sigma=self.gamma)
        else:
            self.model = QN_S3VM_Dense(labeledX, labeledy, unlabeledX, self.random_generator, lam=self.C, lamU=self.lamU)
    ...
    def predict(self, X):
        ...
        # print(X.shape)
        y = numpy.array(self.model.getPredictions(X))
        # y[y == -1] = 0
        return y
