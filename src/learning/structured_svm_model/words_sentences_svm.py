...
""" Sentence features - stack top 5 words by tfidf into a vector for that sentence"""
...
def getFeatVectors(doc, w2v_feat):
    ...
    for d in range(len(doc)):
        ...
        # sent_vec = np.pad(sent_vec, (0, 500 - len(sent_vec)), 'constant', constant_values=0)
        doc_vec.append(np.mean(np.array(sent_vec), axis=0))

    # print(np.array(doc_vec).shape)
...
def get_X_Y_data(forumsData, docs, docs_unlabel, w2v_feat, column, trans_data):
    ...
    for row in range(len(forumsData)):
        # print(row)
        # print(docs[row], forumsData.ix[row, column] )
        # exit()
        if len(docs[row]) == 0:
    ...
    for row in range(len(docs_unlabel)):
        ...
        if len(docs_unlabel[row]) == 0:
            ...
            v = getFeatVectors(docs_unlabel[row], w2v_feat)
        temp_idx = []
        row_indices = []
        for idx in range(len(v)):
            temp_idx.append((row, idx))
    ...
    for idx in range(trans_data): #range(len(unlabel_posts)):
        X_inst.append(unlabel_posts[idx])
