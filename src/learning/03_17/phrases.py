...
def createSentences(data, phrase_sentences, dict):
...
def get_X_Y_data(forumsData, docs, w2v_feat):
    ...
    for row in range(len(forumsData) - 1):
        if forumsData.ix[row, col] == 1.0:
            ...
            if row_feat.shape[1] < 500:
                while row_feat.shape[1] < 500:
                    # row_feat.append(np.zeros(100))
                    # print(row_feat.shape)
                    row_feat = np.hstack((row_feat, np.zeros((1, 100))))

            row_feat = row_feat[:, :500]
            # row_feat =
...
if __name__ == "__main__":
    ...
    docs = createSentences(sentences_data, inputData, dictionary)
