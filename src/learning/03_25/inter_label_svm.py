...
def my_kernel(F_1, F_2):
    K = np.zeros((F_1.shape[0], F_2.shape[0]))
    for idx_1 in range(F_1.shape[0]):
        for idx_2 in range(idx_1, F_2.shape[0]):
            sum_inter_label = 0
            for idx_r in range(relLables.shape[0]):
                sum_inter_label += Y[idx_1, relLables[idx_r]]*Y[idx_2, relLables[idx_r]]
            K[idx_1, idx_2] = 4*(np.square(eta_1) + (np.square(eta_2)*sum_inter_label))*linear_kernel(X[idx_1, :], X[idx_2, :])
            K[idx_1, idx_2] = linear_kernel(F_1[idx_1, :], F_2[idx_2, :])
            K[idx_2, idx_1] = linear_kernel(F_1[idx_1, :], F_2[idx_2, :])
...
def get_matrices(X, Y, curLabel, relLables, C=1000.1):
    ...
    for idx_1 in range(X.shape[0]):
        for idx_2 in range(X.shape[0]):
            ...
            for idx_r in range(relLables.shape[0]):
                sum_inter_label += (Y[idx_1, relLables[idx_r]]*Y[idx_2, relLables[idx_r]])
            K[idx_1, idx_2] = 4*(eta_1**2 + (eta_2**2 * sum_inter_label))*gaussian_kernel(X[idx_1], X[idx_2])
...
def getParams(a, Y, X, relLabels, curLabel):
    ...
    sum_w = np.zeros((rel_labels.shape[0], X.shape[1]))
    for t in range(relLabels.shape[0]):
        for idx in range(a.shape[0]):
            sum_w[t] += (a[idx, 0] * Y[idx, curLabel] * Y[idx, rel_labels[t]] * X[idx, :])
...
if __name__ == "__main__":
    ...
    for col in range(3, 14):
        ...
        for idx_t in range(len(train_fold)):
            ...
            clf = svopt.SVM_cvxopt(kernel=gaussian_kernel, C=1000.1)
            ...
            # avg_precision_r += sklearn.metrics.precision_score(Y_test, Y_predict)
            # print(avg_precision_r)
            # cnt_fold += 1

            # print(sklearn.metrics.precision_score(Y_train, Y_predict))

        # print(avg_precision_r/cnt_fold)
        ...
        #     P = cvxopt.matrix(K)
            ...
            for i in range(len(X_test)):
                ...
                for a_1, sv_y_1, sv_1 in zip(a, sv_y, sv_x):
                    ...
                    s += a_1 * sv_y_1 * gaussian_kernel(X_test[i], sv_1)
                y_predict[i] = s
        ...
        #     # print(Y_predict)
        #     # print(Y_test)
        #     avg_precision += sklearn.metrics.precision_score(Y_test, Y_predict)
        #
        #     # exit()
