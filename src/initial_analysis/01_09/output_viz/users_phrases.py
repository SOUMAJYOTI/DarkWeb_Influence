...
fId = 40
output_dir = '../users_months/' + str(fId)
...
for d in dirs:
    ...
    if d not in months_users_topics:
        months_users_topics[d] = {}
    months = os.listdir(output_dir + '/' + str(d))
    for u in months:
        dict_phrases = open(output_dir + '/' + d + '/' + u + '/dict_phrases.txt', 'r')
        phrase_freq_dict = {}
        cnt_phrases = 0
        ...
        if u not in months_users_topics[d]:
            months_users_topics[d][u] = {}
        for p, freq in sorted_x:
            # if freq <= 1:
            #     continue
            tokens = p.split(' ')
            for t in topic_phrase_dict:
                if t not in months_users_topics[d][u]:
                    months_users_topics[d][u][t] = 0

                if p in topic_phrase_dict[t]:
                    months_users_topics[d][u][t] += 1
                elif len(tokens) > 1:
                    for w in tokens:
    ...
    cnt_months += 1
    print(cnt_months)
# print(months_users_topics)
pickle.dump(months_users_topics, open('m_u_t.pickle', 'wb'))
...
#     plt.ylabel('# Phrases in topic/Total # phrases', size=30, **hfont)
#     plt.title(t, size=30)
