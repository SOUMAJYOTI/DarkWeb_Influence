...
if __name__ == "__main__":
    ...
    for id, row in fPosts.iterrows():
        ...
        for idx in range(len(usersSet)):
            ...
            # sorted_x = sorted(userPosts_dict.items(), key=operator.itemgetter(1))

            cnt_docs = 0
            mDocs = []
            month_docs = list(fPosts_user['postContent'])
            content_seen = []
            ...
            directory = 'month_docs_users/forum/' + str(fId) + '/month_' + str(num_months)
            if not os.path.exists(directory):
                os.makedirs(directory)
            thefile = open(directory + '/user_' + str(usersSet[idx]) + '.txt', 'w')
            for item in mDocs:
