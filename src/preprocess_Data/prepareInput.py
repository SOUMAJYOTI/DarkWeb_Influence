...
def get_X_Y_data(docs, labels, w2v_feat, col, sen_length, vocab_dict, stop):
    ...
    for row in range(len(docs)):
        ...
        for idx_sent in range(len(docs[row])):
            ...
            if len(docs) > 1 and len(sent_index) < 3:
                continue
...
def main():
    ...
    for idx_fold in range(0, len(train_fold)): #range(len(train_fold)):
        ...
        for col in range(2, 12):
            ...
            Y_test_final = np.array([[1, 0]] * X_test_neg.shape[0] + [[0, 1]] * X_test_pos.shape[0])
            ...
            # pickle.dump(vocab_dict, open('../../darkweb_data/5_15/data/vocab_dict_revised.pickle', 'wb'))
